...
public final class Shopping {
    ...
    public func addItemWithoutStorageLocation() {
        itemsWithoutStorageLocation.append(ShoppingItem(id: itemsWithoutStorageLocation.map { $0.id }.max() ?? 99999, title: "", createdAt: Date(), updatedAt: Date(), source: .user, status: .created, storageLocation: nil, product: nil))
        
        Task {
            do {
                let newItems = try await api.addShoppingItem(AddShoppingItemRequest(title: "", source: .user, storageLocation: nil, productId: nil, quantity: nil))
    ...
    public func updateItemByUUID(uuid: UUID, title: String) {
        ...
        Task {
            do {
                try await api.updateShoppingItem(for: itemsWithoutStorageLocation[shoppingItemIndexToUpdate].id, UpdateShoppingItemRequest(title: title))
            } catch {
                print("Updating shopping item failed with error: \(error)")
